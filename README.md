...
```powershell
$env:APP_DATASOURCE_URL="jdbc:postgresql://127.0.0.1:5432/enterprise_approval"
$env:APP_DATASOURCE_USERNAME="postgres"
$env:APP_DATASOURCE_PASSWORD="你的数据库密码"
```

> 默认 JDBC、Vite Dev Server、Vite `/api` 代理都已显式使用 `127.0.0.1`，避免 Windows 上 `localhost` 解析到 IPv6 (`::1`) 而 PG 只在 IPv4 监听导致连不上。如需通过其它机器访问后端，请把 `APP_DATASOURCE_URL` 改成对应实际地址。
...
> 默认账号仅用于本地开发和演示。生产环境必须通过环境变量 `SECURITY_JWT_SECRET` 配置 JWT 密钥并重置所有默认密码；当 `APP_ENVIRONMENT` 不是 `development` / `dev` / `test` 时，未覆盖默认 JWT 密钥会导致后端启动失败。
...
用户：postgres
端口：5432
JDBC：jdbc:postgresql://127.0.0.1:5432/enterprise_approval
```
...
V8__align_default_workflow_templates_with_spec.sql
```
